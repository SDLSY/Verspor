...
class AnimatedCounterView @JvmOverloads constructor(
    ...
    init {
        context.theme.obtainStyledAttributes(
            attrs,
            R.styleable.AnimatedCounterView,
            0, 0
        ).apply {
            try {
                getString(R.styleable.AnimatedCounterView_acv_format)?.let {
                    decimalFormat = DecimalFormat(it)
                }
                getString(R.styleable.AnimatedCounterView_acv_prefix)?.let {
                    prefix = it
                }
                getString(R.styleable.AnimatedCounterView_acv_suffix)?.let {
                    suffix = it
                }
                getInt(R.styleable.AnimatedCounterView_acv_duration, 800).toLong().let {
                    duration = it
                }
            } finally {
                recycle()
