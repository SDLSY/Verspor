...
class DesktopAvatarNarrationService(
    ...
    private suspend fun generateInteractiveNarration(context: PageNarrationContext): AvatarNarration {
        ...
        val cloudNarration = runCatching { generateFromCloud(context) }
            .getOrNull()
    ...
    private fun buildInteractiveFallbackNarration(context: PageNarrationContext): AvatarNarration {
        ...
        val summary = context.userStateSummary
            ...
            .orEmpty()
        val risk = context.riskSummary.takeIf { it.isNotBlank() }
        val text = when {
            !risk.isNullOrBlank() -> "这里先关注$risk。建议先处理当前页面最关键的一步。"
            summary.isNotBlank() -> "这里是${context.pageTitle}，当前重点是$summary。建议先按页面中的第一步继续。"
    ...
    private fun inferSemanticAction(context: PageNarrationContext, text: String): String {
        ...
        return when {
            context.riskSummary.isNotBlank() || text.contains("风险") || text.contains("预警") || lowered.contains("alert") -> "alert"
