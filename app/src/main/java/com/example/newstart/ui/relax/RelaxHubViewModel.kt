...
class RelaxHubViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun executeCurrentPlan() {
        viewModelScope.launch {
            ...
            val now = System.currentTimeMillis()
            val existingTaskId = activeDraftTaskId
            val task = InterventionTaskEntity(
    ...
    fun onBodyZoneSelected(
        ...
        viewModelScope.launch {
            ...
            val zoneLabel = zoneDisplayName(zone, source)
            val currentState = _uiState.value ?: RelaxHubUiState()
            _uiState.postValue(
                currentState.copy(
                    selectedBodyZone = zone.name,
                    selectedZoneDisplayName = zoneLabel,
            ...
            enhanceJob = viewModelScope.launch enhanceLaunch@{
                try {
                    ...
                    if (token != activePlanToken) return@enhanceLaunch
                    ...
                    Log.d(
                        TAG,
                        "onBodyZoneSelected enhanced: zone=$zone, protocol=${llmPlan.protocolType}, duration=$llmDuration"
                    )
                } catch (t: Throwable) {
    ...
    private fun buildRulePlan(
        ...
        return when (zone) {
            ...
            HumanBody3DView.BodyZone.NECK -> {
                InterventionPlanResult(
                    title = "头颈减压呼吸方案",
                    rationale = "头颈紧张提示认知过载，优先降刺激并放慢呼吸节律。",
                    actions = listOf(
                    ...
                    caution = "若出现头晕或胸闷不适，请停止训练。",
                    completionRule = "完成两项动作并记录训练前后压力指数。",
            ...
            HumanBody3DView.BodyZone.UPPER_BACK -> {
                InterventionPlanResult(
                    title = "胸腔节律干预方案",
                    rationale = "胸腔压力信号提示交感负荷偏高，先进行节律呼吸。",
                    actions = listOf(
                        InterventionAction(
                            "4-7-8 呼吸",
            ...
            HumanBody3DView.BodyZone.LOWER_BACK -> {
                InterventionPlanResult(
                    title = "核心放松干预方案",
                    rationale = "核心区域紧张提示副交感抑制，建议使用方块呼吸与放松动作。",
                    actions = listOf(
                        InterventionAction("方块呼吸", "吸-停-呼-停均为 4 秒节律。", 240),
                ...
                InterventionPlanResult(
                    ...
                    actions = listOf(
                        InterventionAction("动态拉伸", "进行肩部与髋部动态拉伸。", 180),
                        InterventionAction("4-6 呼吸", "稳定节律并降低压力反应。", 180)
                    ),
