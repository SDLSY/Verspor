...
import com.example.newstart.databinding.FragmentSleepTrendBinding
...
class SleepTrendFragment : Fragment() {
    ...
    private fun setupUi() {
        binding.chipGroupTimeRange.setOnCheckedStateChangeListener { _, checkedIds ->
            ...
            currentTimeRange = when (checkedIds.first()) {
                R.id.chip_30_days -> TimeRange.LAST_30_DAYS
    ...
    private fun observeData() {
        viewModel.sleepDurationData.observe(viewLifecycleOwner) { data ->
            updateLineChart(binding.chartSleepDuration, data.first, data.second, color(R.color.chart_sleep))
            binding.tvAvgSleepDuration.text = if (data.second.isNotEmpty()) {
                getString(R.string.trend_avg_sleep_format, data.second.average().toDouble())
            } else {
                getString(R.string.trend_avg_placeholder)
            }
        }

        viewModel.heartRateTrendData.observe(viewLifecycleOwner) { data ->
            updateLineChart(binding.chartHeartRate, data.first, data.second, color(R.color.chart_heart))
            binding.tvAvgHeartRate.text = if (data.second.isNotEmpty()) {
                getString(R.string.trend_avg_heart_rate_format, data.second.average().roundToInt())
            } else {
                getString(R.string.trend_avg_heart_rate_placeholder)
            }
        }

        viewModel.bloodOxygenTrendData.observe(viewLifecycleOwner) { data ->
            updateLineChart(binding.chartBloodOxygen, data.first, data.second, color(R.color.chart_oxygen))
            binding.tvAvgBloodOxygen.text = if (data.second.isNotEmpty()) {
                getString(R.string.trend_avg_blood_oxygen_format, data.second.average().roundToInt())
            } else {
                getString(R.string.trend_avg_blood_oxygen_placeholder)
            }
        }

        viewModel.temperatureTrendData.observe(viewLifecycleOwner) { data ->
            updateLineChart(binding.chartTemperature, data.first, data.second, color(R.color.chart_temperature))
            binding.tvAvgTemperature.text = if (data.second.isNotEmpty()) {
                getString(R.string.trend_avg_temperature_format, data.second.average().toDouble())
            } else {
                getString(R.string.trend_avg_temperature_placeholder)
            }
        }

        viewModel.recoveryScoreData.observe(viewLifecycleOwner) { data ->
            updateLineChart(binding.chartRecoveryScore, data.first, data.second, color(R.color.chart_recovery))
            binding.tvAvgRecoveryScore.text = if (data.second.isNotEmpty()) {
                getString(R.string.trend_avg_recovery_format, data.second.average().roundToInt())
            } else {
                getString(R.string.trend_avg_recovery_placeholder)
            }
        }

        viewModel.motionTrendData.observe(viewLifecycleOwner) { data ->
            updateLineChart(binding.chartMotion, data.first, data.second, color(R.color.chart_motion))
            binding.tvAvgMotion.text = if (data.second.isNotEmpty()) {
                getString(R.string.trend_avg_motion_format, data.second.average().toDouble())
            } else {
                getString(R.string.trend_avg_placeholder)
            }
        }
        ...
        viewModel.ppgTrendData.observe(viewLifecycleOwner) { data ->
            updateLineChart(binding.chartPpgTrend, data.first, data.second, color(R.color.chart_ppg))
            binding.tvAvgPpg.text = if (data.second.isNotEmpty()) {
                getString(R.string.trend_ppg_samples, data.second.size)
            } else {
                getString(R.string.trend_ppg_samples_empty)
            }
        }
    ...
    private fun bindPeriodReport(report: HealthPeriodReportUiModel) {
        ...
        bindReportAction(
            ...
            R.string.trend_period_primary_action
        )
        bindReportAction(
            binding.btnReportSecondaryAction,
            report.secondaryAction,
            R.string.trend_period_secondary_action
        )
    }
    ...
    private fun styleRiskBadge(riskLabel: String) {
        val fillColor = when {
            riskLabel.contains("高") -> withAlpha(color(R.color.status_negative), 0.18f)
            riskLabel.contains("中") -> withAlpha(color(R.color.status_warning), 0.18f)
            else -> withAlpha(color(R.color.status_positive), 0.18f)
        }
        val textColor = when {
            riskLabel.contains("高") -> color(R.color.status_negative)
            riskLabel.contains("中") -> color(R.color.status_warning)
            else -> color(R.color.status_positive)
        }
    ...
    private fun setupLineChart(chart: LineChart) {
        ...
        val textColor = ContextCompat.getColor(requireContext(), R.color.md_theme_light_onSurfaceVariant)
    ...
    private fun setupPieChart(chart: PieChart) {
        ChartStyleConfig.applyPieChartStyle(
            ...
            centerText = getString(R.string.sleep_stage_center_text)
        )
    }
