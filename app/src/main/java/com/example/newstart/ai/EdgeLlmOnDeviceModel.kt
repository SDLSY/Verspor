...
object EdgeLlmOnDeviceModel {
    ...
    fun init(context: Context) {
        ...
        val ready = runLlamaCall("initGenerateModel") { LlamaBridge.initGenerateModel(localPath) } ?: false
        if (ready) {
            modelPath = localPath
            initialized = true
    ...
    fun generateDoctorReply(
        ...
        val rawText = runLlamaCall("generateDoctorReply.generateWithContext") {
            LlamaBridge.generateWithContext(systemPrompt, mergedContext, userPrompt)
        }
        ...
        val rawSimple = runLlamaCall("generateDoctorReply.generate") {
            LlamaBridge.generate(compactPrompt)
        }
