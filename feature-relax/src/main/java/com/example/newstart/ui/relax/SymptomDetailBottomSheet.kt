...
import com.example.newstart.feature.relax.databinding.BottomSheetSymptomDetailBinding
...
class SymptomDetailBottomSheet : BottomSheetDialogFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.tvSymptomSheetTitle.text = getString(
            CommonR.string.symptom_sheet_title_format,
            sideLabel(side),
            zoneLabel(zone)
        )
        binding.tvSymptomSheetSubtitle.text = getString(CommonR.string.symptom_sheet_subtitle)
        binding.sliderSeverity.value = 5f
        binding.tvSeverityValue.text = getString(CommonR.string.symptom_sheet_severity_value, 5)
        ...
        binding.sliderSeverity.addOnChangeListener { _, value, _ ->
            binding.tvSeverityValue.text =
                getString(CommonR.string.symptom_sheet_severity_value, value.toInt())
        }
    ...
    private fun saveSelection() {
        ...
        if (symptomLabel.isBlank()) {
            Toast.makeText(
                requireContext(),
                getString(CommonR.string.symptom_sheet_pick_required),
                Toast.LENGTH_SHORT
            ).show()
            return
        }
        val durationLabel = when (binding.chipGroupSheetDuration.checkedChipId) {
            com.example.newstart.feature.relax.R.id.chip_duration_recent_hour ->
                getString(CommonR.string.symptom_duration_recent_hour)

            com.example.newstart.feature.relax.R.id.chip_duration_recent_three_days ->
                getString(CommonR.string.symptom_duration_recent_three_days)

            else -> getString(CommonR.string.symptom_duration_recent_day)
        }
        setFragmentResult(
            RESULT_KEY,
            bundleOf(
    ...
    private fun zoneLabel(zone: SymptomBodyZone): String {
        return when (zone) {
            SymptomBodyZone.HEAD -> getString(CommonR.string.symptom_zone_head)
            SymptomBodyZone.CHEST -> getString(CommonR.string.symptom_zone_chest)
            SymptomBodyZone.ABDOMEN -> getString(CommonR.string.symptom_zone_abdomen)
            SymptomBodyZone.LIMB -> getString(CommonR.string.symptom_zone_limb)
        }
    }

    private fun sideLabel(side: SurfaceSide): String {
        return when (side) {
            SurfaceSide.FRONT -> getString(CommonR.string.symptom_surface_front)
            SurfaceSide.BACK -> getString(CommonR.string.symptom_surface_back)
        }
    }
