...
import com.example.newstart.service.DataCollectionServiceContract
...
class DeviceViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private fun connectDemoDevice(device: DeviceInfo) {
        mockBleManager.connect(device.deviceId) { success ->
            if (success) {
                ...
                android.util.Log.d("DeviceViewModel", "演示设备连接成功")
                ...
                mockBleManager.setOnDataReceived { sensorData ->
                    android.util.Log.d("DeviceViewModel", "收到演示数据: HR=${sensorData.heartRate}, SpO2=${sensorData.bloodOxygen}")
                }
            } else {
    ...
    fun startBackgroundCollection() {
        if (DemoConfig.isDemoMode) {
            _statusMessage.value = "演示模式下无需启动后台采集服务"
            return
        }
        ...
        val intent = DataCollectionServiceContract.createStartIntent(
    ...
    private fun handleSensorData(data: SensorData) {
        ...
        viewModelScope.launch {
            ...
            if (isEstimatedHrv) {
                Log.d("DeviceViewModel", "原始 HRV 缺失，使用估算值：$hrv ms")
            }
    ...
    private fun estimateMotionIntensity(
        ...
        // 融合三类动态指标：线性加速度、角速度和动态变化率，避免静态抖动放大。
