...
export class OpenRouterPrescriptionProvider implements PrescriptionModelProvider {
  ...
  async generate(input: PrescriptionProviderInput): Promise<DailyPrescriptionResponse | null> {
    const apiKey = readAiEnv("OPENROUTER_API_KEY");
    if (!apiKey) {
      return null;
    }
    ...
    try {
      const response = await fetch(resolveChatCompletionsUrl("openrouter"), {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          model: resolveLogicalModelId("openrouter", "text.structured"),
      ...
      if (!response.ok) {
        ...
        console.warn("[Prescription][OpenRouter] non-200 response", {
      ...
      if (!jsonText) {
        console.warn("[Prescription][OpenRouter] empty or non-json content", {
      ...
      if (!parsed.success) {
        console.warn("[Prescription][OpenRouter] schema validation failed", {
      ...
      console.warn("[Prescription][OpenRouter] request failed", {
